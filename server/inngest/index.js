...
export const inngest = new Inngest({ id: "ems-deepa" });
...
const leaveApplicationReminder = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        if (leaveApplication?.status === "PENDING") {
            ...
            // Send reminder email to admn to take action on leave application
            await sendEmail({
...
const attendanceReminderCron = inngest.createFunction(
    { id: "attendance-reminder-cron", triggers: [{ cron: "0 6 * * *" }] }, //6:00 UTC = 11:30 AM IST
    async ({ step }) => {
        ...
        // Step 2: Get all active, non-delted employees
